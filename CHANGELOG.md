...
*   **LLM-кодирование 860 заголовков** через DeepSeek API: отложено до ротации скомпрометированного ключа автором и до устранения transient API error. Скрипт `scratch/theme_coding_baseline.py` готов и применяет keyword-baseline; следующий шаг — добавить LLM-pipe с чтением `.env`.
