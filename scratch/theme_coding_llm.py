...
VALID_L2 = {"vedic", "classical", "medieval", "colonial", "modern", "contemporary", "unspecified"}
VALID_L3 = {"text", "fieldwork", "archive", "artefact", "image", "unspecified"}
VALID_L4 = {"fundamental", "applied", "methodological"}
...
def parse_args():
    ...
    parser.add_argument(
        "--no-merge",
        action="store_true",
        help="Do not rewrite theme_codes_final.csv (rights/publish stay human).",
    )
...
def main():
    ...
    if args.no_merge:
        print("Skip merge (--no-merge): theme_codes_final.csv left untouched.")
    else:
        merge_and_summarize()
