...
*   **Результат:** Главная таблица автоматически фильтрует список из 226 ученых, оставляя только тех, кто аффилирован с учреждениями выбранного города. Это позволяет наглядно оценить вклад региональных вузов в общую индологическую повестку.
...
*   **Результат:** Система мгновенно скрывает московскую часть и выводит список из 132 петербургских индологов, которые участвуют исключительно в Зографских чтениях. Это позволяет изучать закрытые академические школы и локальные научные традиции.
...
3.  **Экспорт данных для веб-интерфейса:** Формирует JavaScript-файл с упакованными структурами:
    ...
    ```
4.  **Компиляция статических страниц:** Генерирует 226 персональных HTML-файлов профилей в `scholars/`:
    ```bash
...
2.  **Выявить ядро сообщества:** Математически определить **39** ключевых индологов, формирующих интеллектуальный мост между петербургской и московской научными школами (перекрестная когорта).
3.  **Оцифровать 899 докладов** (895 уникальных названий): Сохранить и структурировать названия докладов, темы секций, даты проведения, дни недели и институциональные аффилиации участников.
