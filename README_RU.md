...
*   **Видео и уровни обобщения:** Страницы `videos/` показывают полный список YouTube-записей и сопоставления с докладами; страницы `gumilyov/` раскрывают классификацию докладов по микро-, региональному и глобальному уровню. Эти ссылки также встроены в карточки докладов на страницах конференций, городов, институций, рубрик и мезоуровней.
*   **Интерактивный движок сетевого графа (`networks.html`):** Отдельная высокопроизводительная стеклянно-минималистичная (glassmorphic) страница визуализации связей на базе библиотеки **Vis.js**. Она отображает всех 220 ученых как узлы и их совместные доклады/сессионные пересечения как связи. Поддерживает масштабирование, физическую симуляцию графа, поиск и четыре предустановленных режима: *«Коллаборации»* (Collaborations), *«Экосистема»* (Ecosystem), *«Со-присутствие»* (Co-presence) и *«Посещаемость»* (Attendance), а также боковую панель с детальными профилями.
...
*   **Результат:** Главная таблица автоматически фильтрует список из 220 ученых по городскому сигналу в аффилиации. Это позволяет оценить вклад региональных центров в общую индологическую повестку, не отождествляя городскую приписку с работой в местном вузе: для периферийных участников в программах нередко указан только город или внеуниверситетская принадлежность.
...
*   **Результат:** Система мгновенно скрывает московскую часть и выводит список из 129 петербургских индологов, которые участвуют исключительно в Зографских чтениях; аналогично фиксируются 53 участника, встречающиеся только в Рериховских чтениях. Это позволяет изучать закрытые академические школы и локальные научные традиции.
...
3.  **Экспорт данных для веб-интерфейса:** Формирует JavaScript-файл с упакованными структурами:
    ...
    ```
4.  **Компиляция статических страниц:** Генерирует 220 персональных HTML-файлов профилей в `scholars/`:
    ```bash
...
*   **H7 (Метод подсчета долей участия):** *Подтверждена.* Доля докладов с единственным автором превышает 99% во всем 20-летнем массиве. В результате разница между простым и дробным методами институционального подсчета оказывается ничтожно малой и не влияет на общие рейтинги организаций.
...
3.  **Оцифровать 899 авторских участий** (895 уникальных докладов): Сохранить и структурировать названия докладов, темы секций, даты проведения, дни недели и институциональные аффилиации участников.
