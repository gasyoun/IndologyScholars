---
thread_id: 9519137554432
subject: "Nagari Script"
year: 2010
messages: 8
participants: "Deka, Rabin, Nina Schriber, Brahman, Tulasi, deka, Roman Smirnov, Rabin Рабин"
first: 2010-06-05T11:37:37+03:00
last: 2010-07-19T08:03:43+03:00
---

# Nagari Script

> 8 сообщений · 7 участников · 2010-06-05 — 2010-07-19

## 1. Deka, Rabin — 2010-06-05 11:37:37

Уважаемые ученые,

Какие важные меры могут предотвратить падение Nagari-Дерево Сценарий?
Иными словами, Дева-Nagari сценарий подход неправильным.

What critical measures can prevent fall of Nagari-Tree of Script?
In other words, Deva-Nagari script approach is wrong.

Изобразительное с ответом на русском языке!

What do I mean by Nagari-Tree of Script?
Let me explain with the help of Latin-Tree of Script.

Latin-Tree of Script:

Latin (Basic) - http://www.alanwood.net/unicode/basic_latin.html
Latin-1 Supplement - http://www.alanwood.net/unicode/latin_1_supplement.html
Latin Extended-A - http://www.alanwood.net/unicode/latin_extended_a.html
Latin Extended Additional -
http://www.alanwood.net/unicode/latin_extended_b.html
Latin Extended-B - http://www.alanwood.net/unicode/latin_extended_b.html
Latin Extended-C - http://unicode.org/charts/PDF/U2C60.pdf
Latin Extended-D - http://unicode.org/charts/PDF/UA720.pdf

Above, you find all styles of Latin script - including at least 8
century style to today.

Nagari-Tree of Script:

For Nagari-Tree, Unicode Inc has not taken same approach/moral as it
did for Latin-Tree.

Instead, Unicode Inc has encoded from 19 Century style. This style was
designed form Nagari-Tree to write Hindi using printing press. Hindi
is now Indian National Language. Unicode Inc uses the term
Deva-Nagari.

Unfortunately, Unicode Inc made Sanskrit a sub-set of Hindi too.
To see this you need to know how to read a Deva-Nagari Unicode font
file with its project.

Have you visited Buddhist Pagodas in South East China?

In some Pagodas two kinds of inscription are found.
Some scholars call one Nalanda-Script and the other Kamarupa-Script.
Both Nalanda-Script and Kamarupa-Script are styles variation of
Nagari-Tree Script.

If Nagari-Tree of script can be restored, Kamarupa-Script and
Nalanda-Script get encoded automatically. And other styles that are
dropped as obscure.

In other words, Deva-Nagari script approach is wrong.
So what critical measures can prevent fall of Nagari-Tree of Script?

Regards,
Rabin Deka, Ph.D.
1324 S Winchester BL Unit 177
San Jose, California 95128

## 2. Nina Schriber — 2010-06-25 22:53:06

Можете ли вы объяснить, больше на Nagari-Tree сценариев?

Пожалуйста, просвещать о том, как Наланда-скрипт, а также Kamarupa-
скрипт соединяется с Nagari-скрипта!

Сценарий имя должно быть Nagari-скрипта. Это также обсуждался на 13-й
Всемирной конференции санскрите, Эдинбург, Шотландия, 10-14 июля 2006
года.

Можете ли вы Приведены примеры на подходе и моральные различия в
Unicode?

Namaskar!
Нина Шрайбер


On Jun 5, 1:37 am, "Deka, Rabin" <deka.ra...@…> wrote:

Уважаемые ученые,

Какие важные меры могут предотвратить падение Nagari-Дерево Сценарий?
Иными словами, Дева-Nagari сценарий подход неправильным.

What critical measures can prevent fall of Nagari-Tree of Script?
In other words, Deva-Nagari script approach is wrong.

Изобразительное с ответом на русском языке!

What do I mean by Nagari-Tree of Script?
Let me explain with the help of Latin-Tree of Script.

Latin-Tree of Script:

Latin (Basic) - http://www.alanwood.net/unicode/basic_latin.html
Latin-1 Supplement - http://www.alanwood.net/unicode/latin_1_supplement.html
Latin Extended-A - http://www.alanwood.net/unicode/latin_extended_a.html
Latin Extended Additional -
http://www.alanwood.net/unicode/latin_extended_b.html
Latin Extended-B - http://www.alanwood.net/unicode/latin_extended_b.html
Latin Extended-C - http://unicode.org/charts/PDF/U2C60.pdf
Latin Extended-D - http://unicode.org/charts/PDF/UA720.pdf

Above, you find all styles of Latin script - including at least 8
century style to today.

Nagari-Tree of Script:

For Nagari-Tree, Unicode Inc has not taken same approach/moral as it
did for Latin-Tree.

Instead, Unicode Inc has encoded from 19 Century style. This style
was
designed form Nagari-Tree to write Hindi using printing press. Hindi
is now Indian National Language. Unicode Inc uses the term
Deva-Nagari.

Unfortunately, Unicode Inc made Sanskrit a sub-set of Hindi too.
To see this you need to know how to read a Deva-Nagari Unicode font
file with its project.

Have you visited Buddhist Pagodas in South East China?

In some Pagodas two kinds of inscription are found.
Some scholars call one Nalanda-Script and the other Kamarupa-Script.
Both Nalanda-Script and Kamarupa-Script are styles variation of
Nagari-Tree Script.

If Nagari-Tree of script can be restored, Kamarupa-Script and
Nalanda-Script get encoded automatically. And other styles that are
dropped as obscure.

In other words, Deva-Nagari script approach is wrong.
So what critical measures can prevent fall of Nagari-Tree of Script?


Regards,
Rabin Deka, Ph.D.
1324 S Winchester BL Unit 177
San Jose, California 95128

## 3. Brahman — 2010-06-30 14:08:17

On 5 июн, 12:37, "Deka, Rabin" <deka.ra...@…> wrote:
> Уважаемые ученые,
"Уважаемые ученые" - это здорово!
Из ученых тут, пожалуй, только один. Организатор этого интернет-
общества.

## 4. Deka, Rabin — 2010-06-30 22:45:17

Спасибо!
Это большой форум. На самом деле, несколько профессоров пишет в этом форуме.
Я убежден этот форум способен!

Я постараюсь ответить на некоторые вопросы, которые я получил до сих пор.

Рабин Дека


From: Brahman <tvitaly1@…>
Date: 2010/6/30
Subject: Re: Nagari Script
To: "Общество ревнителей санскрита (Sanscrit)" <nagari@…>


 On 5 июн, 12:37, "Deka, Rabin" <deka.ra...@…> wrote:
> Уважаемые ученые,
"Уважаемые ученые" - это здорово!
Из ученых тут, пожалуй, только один. Организатор этого интернет-общества.

## 5. Tulasi — 2010-07-05 02:08:28

Hi Brahman,

How many letters/symbols are used to write Russian?

Tulasi

From: Brahman <tvitaly1@…>
Date: Wed, 30 Jun 2010 04:08:17 -0700 (PDT)
Subject: Re: Nagari Script
To: "Общество ревнителей санскрита (Sanscrit)" <nagari@…>



On 5 июн, 12:37, "Deka, Rabin" <deka.ra...@…> wrote:
> Уважаемые ученые,
"Уважаемые ученые" - это здорово!
Из ученых тут, пожалуй, только один. Организатор этого интернет-
общества.

## 6. deka — 2010-07-16 09:40:12

Новорожденный к Nagari скрипт :
Индии национальный символ валюты.

http://groups-beta.google.com/group/nagari/web/ISO_India-National-Currency-Symbol_15-July-2010.pdf

(A newborn to Nagari script :
India's national currency symbol)

Рабин Дека / Rabin Deka

## 7. Roman Smirnov — 2010-07-16 11:06:15

5 июля 2010 г. 3:08 пользователь Tulasi <tulasird@…> написал:
> Hi Brahman,
>
> How many letters/symbols are used to write Russian?

33 letters


-- 
Roma
---
om namah shivaya

## 8. Rabin Рабин — 2010-07-19 08:03:43

On Jun 25, 12:53 pm, Nina Schriber <nina.schri...@…> wrote:
> Можете ли вы объяснить, больше на Nagari-Tree сценариев?
> http://groups.google.com/group/nagari/browse_thread/thread/87b4554a43886167?hl=en
> Nina Schriber  : Can you explain more on Nagari-Tree scripts?

"Дерево" использования не требуется. Но я использовал его для покрытия
такие качества, как следующее.

"Tree" use is not required. But I used it to cover such features as
follows.

В некоторых случаях то же письмо может иметь более одного стиля. Чтобы
сохранить Nagari-скрипт, мы должны сохранить каждый уникальный стиль
нашел в том же письме. Таким образом, мы будем иметь более чем одного
типа той же буквой.

In some cases the same letter can have more than one style. To save
Nagari-script, we need to save each unique style found in the same
letter. Thus, we will have more than one type of the same letter.

Примеры / Examples:

NAGARI LETTER KHA ( ख )

Это письмо имеет два разных стилях. Оба стиля должны быть
закодированы. Этот подход реализован в случае греческий сценарий или
латинского алфавита.

This letter has two different styles. Both styles should be encoded.
This approach is implemented in the case of the Greek script or
alphabet.

U+03C6  GREEK SMALL LETTER PHI ( φ )
U+03D5  GREEK PHI SYMBOL ( ϕ )

В греческий, 2 PHI символы были закодированы.
Осуществлять же правила. Заповедник Nagari-скрипта.

In Greek, 2 PHI characters were encoded.
Perform the same rule; conserve Nagari-script.

Я отвечу на вопросы позже.
I will answer other questions later.

Рабина
Rabin
