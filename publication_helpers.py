...
def iso9_transliterate(cyrillic_text):
    result = []
    for ch in cyrillic_text:
        result.append(ISO9_CYRILLIC_TO_LATIN.get(ch, ch))
...
# Residual institutions resolved in the Phase-1 data-foundation pass (2026-06-14);
# Wikidata Q-IDs / ROR ids are recorded in authority_ids.json. Each entry is
# (acronym tokens [matched as whole tokens], phrase substrings, canonical key).
_AFFILIATION_ALIASES = [
    ((), ("валиханов",), "ИИЭ Валиханова"),
    (("иэа",), ("этнологии и антропологии",), "ИЭА РАН"),
    (("гаугн",), ("университет гуманитарных наук",), "ГАУГН"),
    (("гии",), ("искусствознани",), "ГИИ"),
    (("рудн", "rudn"), ("дружбы народов",), "РУДН"),
    (("мгппу",), ("психолого-педагог",), "МГППУ"),
    (("исаа",), ("стран азии и африки",), "ИСАА"),
    (("имли",), ("мировой литератур",), "ИМЛИ РАН"),
    (("ия",), ("языкознания",), "ИЯ РАН"),
    (("ulster",), ("ольстер",), "Ольстерский университет"),
    (("мцр",), ("центр рерихов",), "МЦР"),
    ((), ("университет культуры", "институт культуры"), "СПбГИК"),
    (("ииет",), ("истории естествознания",), "ИИЕТ РАН"),
    (("ранхигс",), ("народного хозяйства и государственной службы",), "РАНХиГС"),
    (("красгау",), ("красноярский государственный аграрн",), "КрасГАУ"),
    (("имбт",), ("монголоведения",), "ИМБТ СО РАН"),
    ((), ("калмыцкий научный центр", "кнц ран"), "КНЦ РАН"),
    (("скрябина",), ("скрябин",), "ГММ Скрябина"),
    (("урфу", "уфу"), ("уральский федеральн",), "УрФУ"),
    (("ргпу",), ("герцен",), "РГПУ им. Герцена"),
    (("ростгму",), ("ростовский государственный медиц",), "РостГМУ"),
    (("нан",), ("востоковедения, киев", "крымск"), "ИВ НАН Украины"),
    (("гмв",), ("музей востока",), "ГМВ"),
...
def normalize_affiliation(aff):
    ...
    if "независим" in value or "independent" in value:
        return "Независимые исследователи"
    tokens = set(re.split(r"[\s,/().«»\"'’\-]+", value))
    for acronyms, phrases, canon in _AFFILIATION_ALIASES:
        if any(a in tokens for a in acronyms) or any(p in value for p in phrases):
            return canon
    return None
...
def page_shell(title, description, canonical_path, body, structured_data=None, extra_head="", robots="index, follow", language="ru", custom_og_image=None):
    ...
    if language == "en":
        ...
        more_label = "More"
        more_nav_items = [
            ("Documentation", "/IndologyScholars/docs.html"),
            ...
            ("Talk marks", "/IndologyScholars/voting.html"),
            ("Meso-levels", "/IndologyScholars/meso/"),
            ("Keywords", "/IndologyScholars/keywords/"),
            ("Keyword audit", "/IndologyScholars/keywords/review.html"),
        ...
        more_nav_items = [
            ...
            ("Отметки докладов", "/IndologyScholars/voting.html"),
            ("Мезоуровни", "/IndologyScholars/meso/"),
            ("Ключевые слова", "/IndologyScholars/keywords/"),
            ("Аудит слов", "/IndologyScholars/keywords/review.html"),
